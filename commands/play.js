...
module.exports = {
  ...
  async execute(message, args) {
    ...
    if (!permissions.has("CONNECT"))
      return message.reply("No se puede conectar al canal de voz, por falta de permisos");
    ...
    if (urlValid) {
      ...
      try {
        ...
        if (error.statusCode === 404)
          return message.reply("No pude encontrar esa pista de Soundcloud.").catch(console.error);
        return message.reply("Hubo un error al reproducir la pista de Soundcloud.").catch(console.error);
      }
    } else {
      try {
        ...
        console.error(error);
        return message.reply("No se encontró ningún vídeo con un título que coincidiera.").catch(console.error);
      }
    }
